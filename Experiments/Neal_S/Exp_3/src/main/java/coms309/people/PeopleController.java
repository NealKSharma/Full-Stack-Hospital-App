...
public class PeopleController {
    ...
    // More enhancements than experiment 2

    // Returns people sorted by name
    @GetMapping("/people/sorted")
    public List<Person> getPeopleSorted(@RequestParam(defaultValue = "firstName") String sortBy) {
        List<Person> people = new ArrayList<>(peopleList.values());

        // Bubble sort based on the parameter
        for (int i = 0; i < people.size() - 1; i++) {
            for (int j = 0; j < people.size() - i - 1; j++) {
                ...
                if (sortBy.equals("lastName")) {
                    current = people.get(j).getLastName();
                    next = people.get(j + 1).getLastName();
                } else {
                    current = people.get(j).getFirstName();
                    next = people.get(j + 1).getFirstName();
                }
