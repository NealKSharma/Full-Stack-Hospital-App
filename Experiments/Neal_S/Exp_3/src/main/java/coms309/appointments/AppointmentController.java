...
public class AppointmentController {
    ...
    // Even more additions for third experiment

    // Returns appointments sorted by date
    @GetMapping("/appointments/sorted")
    public List<Appointment> getAppointmentsSorted() {
        List<Appointment> appointments = new ArrayList<>(appointmentList.values());

        // Bubble sort
        for (int i = 0; i < appointments.size() - 1; i++) {
            for (int j = 0; j < appointments.size() - i - 1; j++) {
                String current = appointments.get(j).getDate();
                String next = appointments.get(j + 1).getDate();

                if (current != null && next != null && current.compareTo(next) > 0) {
